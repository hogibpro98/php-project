    ...
    document.addEventListener('dblclick', function (event) {
      ...
      if (scheduleType === null) {
        ...
        scheduleType = event.target
          .closest('#item')
          .getAttribute('data-schedule-type');
        if (scheduleType) {
    ...
    document.addEventListener("drop", (event) => {
      if (event.target.className == 'skeduler-cell') {
        ...
        if (drag_item.className == 'add_menu') {
            ...
            .done(function (data) {
              ...
              if (data) {
                ...
                outText +=
                  ...
                  ' - ' +
                  endTime +
                  '" data-url="/schedule/route_edit/dialog/staff_edit_dialog.php?id=' +
                  stfPlanId +
          ...
          const dendCnvMin =
            parseInt(dendSplit[0]) * 60 + parseInt(dendSplit[1]);
          // 差分を求め新しい終了時間を算出する
...
$(document).ready(function () {
  ...
  $('.month_select').change(function () {
    ...
    let user_id = $(".tgt-unique_id").val();
    queryParams += 'week=' + week;
    queryParams += '&user=' + user_id;
    window.location.href = '/schedule/route_edit/index.php?' + queryParams;
  });
