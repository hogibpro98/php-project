...
if($btnRootEntry && $upRootUserSvcData){
    ...
    // ログテーブルに登録する
    setMultiEntryLog($upRootUserSvcData);
}
...
if (!empty($planIds)) {
    ...
    foreach ($temp as $val) {
        ...
        $val['service_dtl_name'] = isset($svcDtlInfo[$val['service_detail_id']]['name']) ? $svcDtlInfo[$val['service_detail_id']]['name'] : NULL;
...
//        $tgtType = !empty($tgtMst['type']) ? $tgtMst['type'] : NULL;
        $tgtType = isset($mstSvc[$svcId]['type']) ? $mstSvc[$svcId]['type'] : '';
        if(!empty($tgtType)){
