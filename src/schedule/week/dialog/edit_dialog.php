...
<div class="modal new_default sched_default displayed_part cancel_act" style="height:600px;width:950px!important;">
    ...
    <div class="s_detail">
        ...
        <div class="box1">
            ...
            <select id="selServiceName" class="staff validate[required]"  name="<?= $mainPrefix ?>[service_name]" style="width:320px">
                <option value="">選択してください</option>
...
<?php endforeach; ?>
            ...
            $(".btn_append_jippi").click(function () {
                ...
                $.ajax({
                    url: '/schedule/week/ajax/html-actual-cost.php',
                    type: 'GET',
                    dataType: 'html',
                    data: {
                        place: <?=json_encode($placeId)?>,
                        user: <?=json_encode($userId)?>,
                        jippi_i: jippi_i
                    },
...
<?php endforeach; ?>
        ...
        $('#reload-actual-price').click(function() {
            ...
            $.ajax({
                ...
                success: function(response) {
                    $(".content_jippi").eq(0).find("tbody").find("tr").not(':first').remove();
                    $(".content_jippi").eq(0).find("tbody").append(response)
                },
                error: function(xhr, status, error) {
                    console.log('ERROR: ' + error);
                }
