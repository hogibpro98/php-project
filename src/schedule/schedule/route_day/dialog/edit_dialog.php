    ...
    <div class="add_sub content_jippi" style="border-top: 0;padding-top: 0;">
        <table>
            ...
            <?php if (empty($dispData['jippi'])) : ?>
                ...
                <td class="type">
                    <b class="sm">種類</b>
                    <select name="<?= $jpiPrefix ?>[0][type]" class="uis_type">
                        <option value="">選択してください</option>
                ...
                <td class="item">
                    <b class="sm">項目名称</b>
                    <select class="cngOffice uis_name" name="<?= $jpiPrefix ?>[0][uninsure_id]">
                        <option value="">選択してください</option>
                ...
                <td class="price" style="width:60px;">
                    <b class="sm">単価最大7桁</b>
                    <input type="text" class="validate[maxSize[7],onlyNumberSp] uis_price" name="<?= $jpiPrefix ?>[0][price]" value="<?= isset($jippiList['price']) ? $jippiList['price'] : '' ?>" style="width:85px">
                </td>
                <td class="tax">
                    <b class="sm">消費税<br>区分</b>
                    <select name="<?= $jpiPrefix ?>[0][zei_type]" class="uis_zei_type">
                ...
                <td class="sales_tax">
                    <b class="sm">消費税率</b>
                    <input type="text" class="validate[maxSize[2],onlyNumberSp] uis_rate" name="<?= $jpiPrefix ?>[0][rate]" value="<?= isset($jippiList['rate']) ? $jippiList['rate'] : '' ?>" style="width:60px;"><span>%</span>
                </td>
                <td class="d_cate">
                    <b class="sm">控除区分</b>
                    <select name="<?= $jpiPrefix ?>[0][subsidy]" class="uis_subsidy">
            ...
            <?php else : ?>
                <?php foreach ($dispData['jippi'] as $jippiList) : ?>
                    ...
                    <td class="type">
                        <b class="sm">種類</b>
                        <select name="<?= $jpiPrefix ?>[<?= $jippiId ?>][type]" class="uis_type">
                            <option value="">選択してください</option>
                            <?php foreach ($unInsType['type'] as $type => $dummy) : ?>
                    ...
                    <td class="item">
                        <b class="sm">項目名称</b>
                        <select class="cngOffice uis_name" name="<?= $jpiPrefix ?>[<?= $jippiId ?>][uninsure_id]">
                            <option value="">選択してください</option>
                            <?php foreach ($uisList as $type => $uisList2) : ?>
                                <?php foreach ($uisList2 as $uisId => $uisData) : ?>
                    ...
                    <td class="price">
                        <b class="sm">単価最大7桁</b>
                        <input type="text" class="validate[maxSize[7],onlyNumberSp] uis_price" name="<?= $jpiPrefix ?>[<?= $jippiId ?>][price]" value="<?= $jippiList['price'] ?>" style="width:85px" maxlength="7" placeholder="半角数字7桁">
                    </td>
                    <td class="tax">
                        <b class="sm">消費税<br>区分</b>
                        <select name="<?= $jpiPrefix ?>[<?= $jippiId ?>][zei_type]" class="uis_zeiType">
                            <?php foreach ($unInsType['zei_type'] as $zeiType => $dummy) : ?>
                    ...
                    <td class="sales_tax">
                        <b class="sm">消費税率</b>
                        <input type="text" class="validate[maxSize[2],onlyNumberSp] uis_rate" name="<?= $jpiPrefix ?>[<?= $jippiId ?>][rate]" value="<?= isset($jippiList['rate']) ? $jippiList['rate'] : '' ?>" style="width:60px;" maxlength="2" placeholder="半角数字2桁"><span>%</span>
                    </td>
                    <td class="d_cate">
                        <b class="sm">控除区分</b>
                        <select name="<?= $jpiPrefix ?>[<?= $jippiId ?>][subsidy]" class="uis_subsidy">
                            <?php foreach ($unInsType['subsidy'] as $subsidy => $dummy) : ?>
            ...
            <ol id="dtl_root">
                <?php if (empty($serviceData)) : ?>
                    <li>
                        ...
                        <select name="<?= $svcPrefix ?>[0][start_time_h]" style="width:60px;">
                            <?php foreach ($selHour as $val) : ?>
                                <option value="<?= $val ?>"><?= $val ?></option>
                            <?php endforeach; ?>
                        </select>
                        <small>：</small>
                        <select name="<?= $svcPrefix ?>[0][start_time_m]" style="width:60px;">
                            <?php foreach ($selMinutes as $val) : ?>
                                <option value="<?= $val ?>"><?= $val ?></option>
                            <?php endforeach; ?>
                        </select>
                        <small>～</small>
                        <select name="<?= $svcPrefix ?><?= $svcPrefix ?>[0][end_time_h]" style="width:60px;">
                            <?php foreach ($selHour as $val) : ?>
                                <option value="<?= $val ?>"><?= $val ?></option>
                            <?php endforeach; ?>
                        </select>
                        <small>：</small>
                        <select name="<?= $svcPrefix ?><?= $svcPrefix ?>[0][end_time_m]" style="width:60px;">
                            <?php foreach ($selMinutes as $val) : ?>
                                <option value="<?= $val ?>"><?= $val ?></option>
                            <?php endforeach; ?>
                        </select>
                        <select class="cngOffice" name="<?= $svcPrefix ?>[0][service_detail_id]" style="width:200px;">
                            <option value="">選択してください</option>
                ...
                <?php else : ?>
                    <?php foreach ($serviceData as $svcData) : ?>
                        <li>
                            ...
                            <input type="hidden" name="<?= $svcPrefix ?>[<?= $svcId ?>][unique_id]" value="<?= $svcId ?>">
                            <input type="hidden" name="<?= $svcPrefix ?>[<?= $svcId ?>][user_plan_id]" value="<?= $planId ?>">
                            <select name="<?= $svcPrefix ?>[<?= $svcId ?>][start_time_h]" style="width:60px;">
                                <?php foreach ($selHour as $val) : ?>
                            ...
                            <small>：</small>
                            <select name="<?= $svcPrefix ?>[<?= $svcId ?>][start_time_m]" style="width:60px;">
                                <?php foreach ($selMinutes as $val) : ?>
                                    <?php $selected = strpos($svcData['start_time'], ":" . $val) !== false ? ' selected' : ""; ?>
                                    <option value="<?= $val ?>" <?= $selected ?>><?= $val ?></option>
                                <?php endforeach; ?>
                            ...
                            <small>～</small>
                            <select name="<?= $svcPrefix ?>[<?= $svcId ?>][end_time_h]" style="width:60px;">
                                <?php foreach ($selHour as $val) : ?>
                            ...
                            <small>：</small>
                            <select name="<?= $svcPrefix ?>[<?= $svcId ?>][end_time_m]" style="width:60px;">
                                <?php foreach ($selMinutes as $val) : ?>
                                    <?php $selected = strpos($svcData['end_time'], ":" . $val) !== false ? ' selected' : ""; ?>
                                    <option value="<?= $val ?>" <?= $selected ?>><?= $val ?></option>
                                <?php endforeach; ?>
                            </select>
                            <select class="cngOffice" name="<?= $svcPrefix ?>[<?= $svcId ?>][service_detail_id]" style="width:200px;">
                                <option value="">選択してください</option>
                                <?php foreach ($svcDtlMst as $type => $svcDtlMst2) : ?>
                                    <?php foreach ($svcDtlMst2 as $tgtId => $val) : ?>
...
<?php endforeach; ?>
            ...
            $(".btn_append_jippi").click(function () {
                ...
                newRow += '      <select class="cngOffice uis_type" name="<?= $jpiPrefix ?>[' + jippiNewCnt + '][type]">';
                newRow += '        <option value="">選択してください</option>';
<?php foreach ($unInsType['type'] as $type => $dummy) : ?>
...
<?php endforeach; ?>
                ...
                newRow += '      <select class="cngOffice uis_name" name="<?= $jpiPrefix ?>[' + jippiNewCnt + '][uninsure_id]">';
                newRow += '        <option value="">選択してください</option>';
<?php foreach ($uisList as $type => $uisList2) : ?>
    <?php foreach ($uisList2 as $uisId => $uisData) : ?>
...
<?php endforeach; ?>
                ...
                newRow += '      <input type="text" class="validate[maxSize[7],onlyNumberSp] uis_price" name="<?= $jpiPrefix ?>[' + jippiNewCnt + '][price]" value="<?= isset($jippiList['price']) ? $jippiList['price'] : '' ?>" style="width:85px" placeholder="半角数字7桁">';
                newRow += '    </td>';
                newRow += '    <td class="tax">';
                newRow += '      <b class="sm">消費税<br>区分</b>';
                newRow += '      <select name="<?= $jpiPrefix ?>[' + jippiNewCnt + '][zei_type]" class="uis_zei_type>';
<?php foreach ($unInsType['zei_type'] as $zeiType => $dummy) : ?>
...
<?php endforeach; ?>
                ...
                newRow += '      <input type="text" class="validate[maxSize[7],onlyNumberSp] uis_rate" name="<?= $jpiPrefix ?>[' + jippiNewCnt + '][rate]" value="<?= isset($jippiList['rate']) ? $jippiList['rate'] : '' ?>" style="width:60px;" placeholder="半角数字2桁"><span>%</span>';
                newRow += '    </td>';
                newRow += '    <td class="d_cate">';
                newRow += '      <b class="sm">控除区分</b>';
                newRow += '      <select name="<?= $jpiPrefix ?>[' + jippiNewCnt + '][subsidy]" class="uis_subsidy">';
<?php foreach ($unInsType['subsidy'] as $subsidy => $dummy) : ?>
...
<?php endforeach; ?>
            ...
            $(".btn_append_service").click(function () {
                ...
                var newRow = '';
                newRow += '<li>';
                newRow += '  <select name="<?= $svcPrefix ?>[' + svcNewCnt + '][start_time_h]" style="width:60px;">';
<?php foreach ($selHour as $val) : ?>
                    newRow += '      <option value="<?= $val ?>"><?= $val ?></option>';
<?php endforeach; ?>
                newRow += '  </select>';
                newRow += '  <small>：</small>';
                newRow += '  <select name="<?= $svcPrefix ?>[' + svcNewCnt + '][start_time_m]" style="width:60px;">';
<?php foreach ($selMinutes as $val) : ?>
                    newRow += '      <option value="<?= $val ?>"><?= $val ?></option>';
<?php endforeach; ?>
                newRow += '  </select>';
                newRow += '  <small>～</small>';
                newRow += '  <select name="<?= $svcPrefix ?>[' + svcNewCnt + '][end_time_h]" style="width:60px;">';
<?php foreach ($selHour as $val) : ?>
                    newRow += '      <option value="<?= $val ?>"><?= $val ?></option>';
<?php endforeach; ?>
                newRow += '  </select>';
                newRow += '  <small>：</small>';
                newRow += '  <select name="<?= $svcPrefix ?>[' + svcNewCnt + '][end_time_m]" style="width:60px;">';
<?php foreach ($selMinutes as $val) : ?>
                    newRow += '      <option value="<?= $val ?>"><?= $val ?></option>';
<?php endforeach; ?>
                newRow += '  </select>';
                newRow += '  <select class="cngOffice" name="<?= $svcPrefix ?>[' + svcNewCnt + '][service_detail_id]" style="width:200px;">';
                newRow += '     <option value="">選択してください</option>';
<?php foreach ($svcDtlMst as $type => $svcDtlMst2) : ?>
    <?php foreach ($svcDtlMst2 as $tgtId => $val) : ?>
