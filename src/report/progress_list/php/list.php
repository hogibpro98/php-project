...
$search['importantly'] = !empty($search['importantly']) ? $search['importantly'] : NULL;
$search['type1']       = !empty($search['type1'])       ? $search['type1']       : NULL;
$search['type2']       = !empty($search['type2'])       ? $search['type2']       : NULL;
...
$progressReportMst = getCode('経過記録');
...
if(!empty($search['type1'])){
    $where['type1 ='] = $search['type1'];
}
if(!empty($search['type2'])){
    $where['type2 ='] = $search['type2'];
}
...
foreach($temp as $idx => $val){
    ...
    // 重要フラグ
    if(isset($search['importantly'])){
        if($dat['importantly'] !== '重要'){
            continue;
        } 
    }
