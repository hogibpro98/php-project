 ...
 * -----------------------------------------------------------------------
 */
function select($table, $target = '*', $condition = array(),
    ...
    if (!empty($where)){$sql .= sprintf(" WHERE %s",implode(" AND ",$where));}
    if (!empty($orderBy)){$sql .= sprintf(" ORDER BY %s",$orderBy);}
    if (!empty($limit)){$sql .= sprintf(" LIMIT %s",$limit);}
    /*--DB操作-----------------------------------------*/
    $pdo = connect();
 ...
 * -----------------------------------------------------------------------
 */
function multiSelect($table, $jointables, $target = '*', $condition = array(), $joinconditions = array(),
    ...
    foreach ($jointables as $jointable) {
        ...
        if (!empty($joinon)){$sql .= sprintf(" ON %s",implode(" AND ",$joinon));}
    ...
    if (!empty($orderBy)){$sql .= sprintf(" ORDER BY %s",$orderBy);}
    if (!empty($limit)){$sql .= sprintf(" LIMIT %s",$limit);}
    if (!empty($offset)){$sql .= sprintf(" OFFSET %s",$offset);}
...
function update($pdo,$table,$setData,$term){
    ...
    $result = $stmt->execute();
    $res = isset($setData['unique_id']) ? $setData['unique_id'] : TRUE;
    return $res;
}
