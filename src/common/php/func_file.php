...
function formatUploadFiles($files,
                           $overlap  = true,
...
function checkOverlapPath($path,
                          $copyMode = true,
                          $retry = 1000
){
    $res = array();
...
function download($dir, $fileName) {
    ...
    // ファイルを読み込みできない場合はエラー
    if (!is_readable($filePath)) { die($filePath); }
