...
?>
<div class="cont_staff cancel_act">
    <div class="tit">スタッフ選択</div>
    <div>
        <span class="label_t">氏名漢字／カナ</span>
        <input type="text" class="searchKanaSt1" value="">
        <button type="button" class="btnDlgSearchSt1">検索</button>
    </div>
        ...
        <tbody>
            <?php foreach ($stfList as $stfId => $val): ?>
                <tr>
                    ...
                    <td class="tgtSearchVal1" style="display:none;"><?= $val['staff_id'] ?></td>
                    <td class="tgtSearchVal1"><?= $val['last_name'] . $val['first_name'] ?></td>
                    <td class="tgtSearchVal1" style="display:none;"><?= $val['last_kana'] . $val['first_kana'] ?></td>
                    <td><?= $val['license1'] ?></td>
...
<script>
    $(function () {
        ...
        // モーダルから選択
        $(".cont_staff").find("table button").on("click", function () {
            // 各種データ取得
            var stf_id = $(this).data("stf_id");
            var stf_cd = $(this).data("stf_cd");
            var stf_name = $(this).data("stf_name");

            /* ※表示先のinputには .tgt-stf_id .tgt-stf_name のクラスを指定しておくこと。 */

            $(".tgt-stf_id").val(stf_id);
            $(".tgt-stf_cd").val(stf_cd);
            $(".tgt-stf_name").val(stf_name);
        ...
        // 氏名（漢字／カナ）検索
        $(".btnDlgSearchSt1").on("click", function () {
            var kana = $(".searchKanaSt1").val();
            if (kana) {
                // 一旦絞込を解除する
                $(".tgtSearchVal1").each(function () {
