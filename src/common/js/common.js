 ...
 * -----------------------------------------------------------------------
 */
function addRecord(templateId='template', changeStrAry={}, tgtDomId='f-detailBody'){
    
    var originHtml   = $('#'+templateId).html();
    var nowSeq       = $('#'+templateId).data('seq');
    var nextSeq      = nowSeq+1;
    ...
    var tempAry = JSON.parse(changeStrAry);
    
    console.log(tempAry);
    var tgtHtml = changeAry(originHtml, tempAry);
    
    tgtHtml = changeStr(tgtHtml, '=""', '');
    tgtHtml = changeStr(tgtHtml, 'seq', nextSeq);
    
    var obj = $(tgtHtml).appendTo($('#'+tgtDomId));
 ...
 * -----------------------------------------------------------------------
 */
function copyRecord(templateId='template', changeStrAry={}, tgtDomObj='f-detailBody'){
    ...
    var obj = $(tgtHtml).insertAfter($(tgtDomObj).closest('tr'));
    ...
    obj.find('input').each(function(index){
        var defaultVal = $(tgtDomObj).closest('tr').find('input').eq(index).val();
        $(this).val(defaultVal);
    });
    
    // textarea要素対応
    obj.find('textarea').each(function(index){
        var defaultVal = $(tgtDomObj).closest('tr').find('textarea').eq(index).val();
        $(this).val(defaultVal);
    });
...
$(function() {
    if($(".tab").length){
        $(".tab-btn li,.tab-btn tr").on("click",function(){
            ...
            var index = $(parentTab).find('.tab-btn li,.tab-btn tr').index(this);
            ...
            $(parentTab).find(".tab-main").removeClass("is-show").eq(index).addClass("is-show"); 
        });
    };
});
...
//                    ['fontfamily'],                                   // フォント変更
            ...
            plugins: {
                    colors: {
                        colorList: [
                            '000000', 'ffffff', 'ffc0cb', 'ee82ee', 'ffff00', 'ffa500', 'd2691e', '00ff00', '0000ff', '00ffff', 'ff1493',
                            '696969', 'fffafa', 'ffccd5', 'edbeed', 'ffffcc', 'ffedcc', 'd1b9a7', '00cc00', 'ccccff', 'ccffff', 'ffb114',
                            '808080', 'f8f8ff', 'ff99aa', 'ed8eed', 'ffff99', 'ffdb99', 'd1a07d', '009900', '9999ff', '99ffff', '34ff14',
                            'a9a9a9', 'fffaf0', 'ff6680', 'ed5fed', 'cccc00', 'ffc966', 'd18854', '006600', '6666ff', '66ffff', '14efff',
                            'c0c0c0', 'faf0e6', 'ff3355', 'ed2fed', '999900', 'ffb833', 'd1702a', '008000', '3333ff', '33ffff', '5314ff',
                            'd3d3d3', 'faebd7', 'ff002b', 'ed00ed', '666600', 'ffa500', 'd15700', '003300', '0000ff', '00ffff', '8214ff',
                        ],
                        displayAsList: true
                    },
                    fontsize: {
...
$(function(){
    ...
    $("submit[name='btnDelOfc1'],button[name='btnDelOfc1']").on("click",function(){
        var result = window.confirm('削除してもよろしいですか？');
        if(!result){
            // いいえ押下時、Submit阻止
            return false;
    ...
    $("submit[name='btnDelOfc2'],button[name='btnDelOfc2']").on("click",function(){
        var result = window.confirm('削除してもよろしいですか？');
        if(!result){
            // いいえ押下時、Submit阻止
            return false;
    ...
    $("submit[name='btnDelIns2'],button[name='btnDelIns2']").on("click",function(){
        var result = window.confirm('削除してもよろしいですか？');
        if(!result){
            // いいえ押下時、Submit阻止
            return false;
    ...
    $("submit[name='btnDelIns3'],button[name='btnDelIns3']").on("click",function(){
        var result = window.confirm('削除してもよろしいですか？');
        if(!result){
            // いいえ押下時、Submit阻止
            return false;
    ...
    $("submit[name='btnDelIns4'],button[name='btnDelIns4']").on("click",function(){
        var result = window.confirm('削除してもよろしいですか？');
        if(!result){
            // いいえ押下時、Submit阻止
            return false;
    ...
    $("submit[name='btnDelHsp'],button[name='btnDelHsp']").on("click",function(){
        var result = window.confirm('削除してもよろしいですか？');
        if(!result){
            // いいえ押下時、Submit阻止
            return false;
    ...
    $("submit[name='btnDelDrg'],button[name='btnDelDrg']").on("click",function(){
        var result = window.confirm('削除してもよろしいですか？');
        if(!result){
            // いいえ押下時、Submit阻止
            return false;
    ...
    $("submit[name='btnDelSvc'],button[name='btnDelSvc']").on("click",function(){
        var result = window.confirm('削除してもよろしいですか？');
        if(!result){
            // いいえ押下時、Submit阻止
            return false;
    ...
    $("submit[name='btnDelInt'],button[name='btnDelInt']").on("click",function(){
        var result = window.confirm('削除してもよろしいですか？');
        if(!result){
            // いいえ押下時、Submit阻止
            return false;
    ...
    $("submit[name='btnDelImg'],button[name='btnDelImg']").on("click",function(){
        var result = window.confirm('削除してもよろしいですか？');
        if(!result){
            // いいえ押下時、Submit阻止
            return false;
    ...
    $("submit[name='btnDelFcl'],button[name='btnDelFcl']").on("click",function(){
        var result = window.confirm('削除してもよろしいですか？');
        if(!result){
            // いいえ押下時、Submit阻止
            return false;
    ...
    $("submit[name='btnDelFml'],button[name='btnDelFml']").on("click",function(){
        var result = window.confirm('削除してもよろしいですか？');
        if(!result){
            // いいえ押下時、Submit阻止
            return false;
    ...
    $("submit[name='btnDelPrb'],button[name='btnDelPrb']").on("click",function(){
        var result = window.confirm('削除してもよろしいですか？');
        if(!result){
            // いいえ押下時、Submit阻止
            return false;
    ...
    $("submit[name='btnDelUserPlan'],button[name='btnDelUserPlan']").on("click", function () {
        var result = window.confirm('削除してもよろしいですか？');
        if (!result) {
            // いいえ押下時、Submit阻止
            return false;
    ...
    $("submit[name='btnDelStfPlan'],button[name='btnDelStfPlan']").on("click", function () {
        var result = window.confirm('削除してもよろしいですか？');
        if (!result) {
            // いいえ押下時、Submit阻止
            return false;
    ...
    $("submit[name='btnDelUserRcd'],button[name='btnDelUserRcd']").on("click", function () {
        var result = window.confirm('削除してもよろしいですか？');
        if (!result) {
            // いいえ押下時、Submit阻止
            return false;
    ...
    $("submit[name='btnDelStfRcd'],button[name='btnDelStfRcd']").on("click", function () {
        var result = window.confirm('削除してもよろしいですか？');
        if (!result) {
            // いいえ押下時、Submit阻止
            return false;
    ...
    $("submit[name='btnCancelUser'],button[name='btnCancelUser']").on("click", function () {
        var result = window.confirm('予定をキャンセルしてもよろしいですか？');
        if (!result) {
            // いいえ押下時、Submit阻止
            return false;
        }
    ...
    $("submit[name='btnCxlUserSvc'],button[name='btnCxlUserSvc']").on("click", function () {
        var result = window.confirm('予定をキャンセルしてもよろしいですか？');
        if (!result) {
            // いいえ押下時、Submit阻止
            return false;
    ...
    $("submit[name='btnCxlUser'],button[name='btnCxlUser']").on("click", function () {
        var result = window.confirm('予定をキャンセルしてもよろしいですか？');
        if (!result) {
            // いいえ押下時、Submit阻止
            return false;
    ...
    $("submit[name='btnCxlStf'],button[name='btnCxlStf']").on("click", function () {
        var result = window.confirm('予定をキャンセルしてもよろしいですか？');
        if (!result) {
            // いいえ押下時、Submit阻止
            return false;
    ...
    $("submit[name='btnCancelStf'],button[name='btnCancelStf']").on("click", function () {
        var result = window.confirm('予定をキャンセルしてもよろしいですか？');
        if (!result) {
            // いいえ押下時、Submit阻止
            return false;
    ...
    // 指示書 PDF削除
    $("submit[name='btnDelPdf'],button[name='btnDelPdf']").on("click", function () {
        var result = window.confirm('削除してもよろしいですか？');
        if (!result) {
            // いいえ押下時、Submit阻止
            return false;
    ...
    $("submit[name='btnRead'],button[name='btnRead']").on("click", function () {
        var result = window.confirm('既読にしますか？');
        if (!result) {
            // いいえ押下時、Submit阻止
            return false;
        }
    ...
    // 展開ボタン確認
    $("submit[name='btnMakePlan'],button[name='btnMakePlan']").on("click", function () {
        var result = window.confirm('展開処理を実行してもよろしいですか？');
        if (!result) {
            // いいえ押下時、Submit阻止
            return false;
        }
    ...
    // 利用者一覧 展開ボタン確認
    $("submit[name='btnMakePlanAll'],button[name='btnMakePlanAll']").on("click", function () {
        var result = window.confirm('展開処理を実行してもよろしいですか？');
        if (!result) {
            // いいえ押下時、Submit阻止
            return false;
...
function enterImpersonateTab(field){
    var oObject = $(field).find('input:not(:button):not([type="radio"]),select');
    var objectAry = $.makeArray($(oObject));
    ...
    $(oObject).on('keypress',function(e){
        ...
        if (c == 13) {
            ...
            for (i=0; i<nLength; i++){
                if ($(objectAry[cNext]).attr("readonly") === "readonly"
...
function enterImpersonateTab4login(){
    var oObject = $('form').find('input,button');
    var objectAry = $.makeArray($(oObject));
...
function zipComposition(str) {
    return str.replace(/[^0-9０-９]/g, '').replace(/[Ａ-Ｚａ-ｚ０-９]/g, function(s) {
...
function seireki(warekiYear) {
    ...
    var matches = warekiYear.match('^(明治|大正|昭和|平成|令和)([0-9０-９]+)年$');

    if(matches) {

        var eraName = matches[1];
        var year = parseInt(matches[2].replace(/[０-９]/g, function(match){
