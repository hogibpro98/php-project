...
<div class="dynamic_modal sched_details add_memo_details rt_memo cancel_act" style="left:50%;width:450px;display:block;">
    ...
    <div class="s_detail">
        <div class="box1">
            <p class="mid">ルート</p>
            <input type="hidden" name="<?= $mainPrefix ?>[<?= $planId ?>][root_id]" value="<?= $dispData['root_id'] ?>">
            <input type="hidden" name="<?= $mainPrefix ?>[<?= $planId ?>][staff_id]" value="<?= $dispData['staff_id'] ?>">
            <p name="<?= $mainPrefix ?>[<?= $planId ?>][root_name]"><?= $dispData['root_name'] ?></p>
        </div>
        <div class="box1">
            <p class="mid">日付/時刻</p>
    ...
    <div class="s_constrols">
        ...
        <p>
            <button type="submit" class="btn save" name="btnNewStfPlan" value="true">保存</button>
        </p>
    </div>
    <div class="update">
        最終更新:
