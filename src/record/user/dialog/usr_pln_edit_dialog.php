...
foreach ($temp as $val) {
    ...
    $val['staff_code']  = getStaffCode($val['update_user']);
        ...
        <table>
            ...
            <?php if (empty($dispData['jippi'])) : ?>
                <tr>
                    ...
                    <td class="sales_tax">
                        <b class="sm">消費税率</b>
                        <input id="jippiUisRate<?= $jippi_i ?>" type="text" class="validate[maxSize[2],custom[onlyNumberSp]] uis_rate" name="<?= $jpiPrefix ?>[0][rate]" value="<?= isset($jippiList['rate']) ? $jippiList['rate'] : '' ?>" style="width:60px;" maxlength="2" placeholder="半角数字2桁"><span>%</span>
                    </td>
                    <td class="d_cate">
                        <b class="sm">控除区分</b>
                    ...
                    <?php foreach ($serviceData as $svcData) : ?>
                        ...
                        <input type="hidden" name="<?= $svcPrefix ?>[<?= $svcId ?>][user_plan_id]" value="<?= $planId ?>">
                            ...
                            <select class="cngOffice validate[required]" name="<?= $svcPrefix ?>[<?= $svcId ?>][service_detail_id]" style="width:200px;">
                                <option value="">選択してください</option>
                                <?php foreach ($svcDtlMst as $type => $svcDtlMst2) : ?>
                                    <?php if (!empty($mainData['service_name']) && $mainData['service_name'] != $type){continue;} ?>
                                    <?php foreach ($svcDtlMst2 as $tgtId => $val) : ?>
...
<?php endforeach; ?>
                ...
                newRow += '      <input id="jippiPrice' + jippi_i + '" type="text" class="validate[maxSize[7],custom[onlyNumberSp]] uis_price" name="<?= $jpiPrefix ?>[' + jippiNewCnt + '][price]" value="" style="width:85px" maxlength="7" placeholder="半角数字7桁">';
                newRow += '    </td>';
                newRow += '    <td class="tax">';
