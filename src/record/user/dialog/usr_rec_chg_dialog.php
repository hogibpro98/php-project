...
$unInsType = array();
$unInsInfo = array();
$tgtData['main'] = initTable('dat_user_record');
...
<div class="dynamic_modal new_default sched_default displayed_part cancel_act" style="height:600px;width:950px!important;overflow: scroll!important;overflow-y: auto;overscroll-behavior-y: contain;top:60%;">
    ...
    <div class="sched_tit">利用者スケジュール予定（実績変更）</div>
    <!-- 基本情報表示エリア:start -->
    <div class="s_detail">
        ...
        <?php $mainPrefix = "upUserRcd[" . 0 . "]"; ?>
        <?php $addPrefix = "upUserRcdAdd[" . 0 . "]"; ?>
        <?php $jpiPrefix = "upUserRcdJpi[" . 0 . "]"; ?>
        <?php $svcPrefix = "upUserRcdSvc[" . 0 . "]"; ?>
        <?php $detail_i = 1; ?>
        <?php $jippi_i = 1; ?>
        <div class="box1">
...
<?php $zeiTypeList = $codeList['保険外マスタ']['税区分']; ?>
            ...
            <ol id="dtl_root">
                <?php if (empty($serviceData)) : ?>
                    <li>
                        ...
                        <small>：</small>
                        <select class="validate[required, custom[notEqualTime]]" id="end_time_m_<?= $detail_i ?>" data-index="<?= $detail_i ?>" name="<?= $svcPrefix ?><?= $svcPrefix ?>[0][end_time_m]" style="width:60px;">
                            <?php foreach ($selMinutes as $val) : ?>
                                <option value="<?= $val ?>"><?= $val ?></option>
                            <?php endforeach; ?>
                        </select>
                ...
                <?php else : ?>
                    ...
                    <?php foreach ($serviceData as $svcData) : ?>
                        <li>
                        <?php $i++; ?>
                            <select class="validate[required]" id="start_time_h_<?= $detail_i ?>" name="<?= $svcPrefix ?>[<?= $i ?>][start_time_h]" style="width:60px;">
                                <?php foreach ($selHour as $val) : ?>
                            ...
                            <small>：</small>
                            <select class="validate[required]" id="start_time_m_<?= $detail_i ?>" name="<?= $svcPrefix ?>[<?= $i ?>][start_time_m]" style="width:60px;">
                                <?php foreach ($selMinutes as $val) : ?>
                                    <?php $selected = strpos($svcData['start_time'], ":" . $val) !== false ? ' selected' : ""; ?>
                                    <option value="<?= $val ?>" <?= $selected ?>><?= $val ?></option>
                                <?php endforeach; ?>
                            ...
                            <small>～</small>
                            <select class="validate[required]" id="end_time_h_<?= $detail_i ?>" name="<?= $svcPrefix ?>[<?= $i ?>][end_time_h]" style="width:60px;">
                                <?php foreach ($selHour as $val) : ?>
                            ...
                            <small>：</small>
                            <select class="validate[required, custom[notEqualTime]]" id="end_time_m_<?= $detail_i ?>" data-index="<?= $detail_i ?>" name="<?= $svcPrefix ?>[<?= $i ?>][end_time_m]" style="width:60px;">
                                <?php foreach ($selMinutes as $val) : ?>
                                    <?php $selected = strpos($svcData['end_time'], ":" . $val) !== false ? ' selected' : ""; ?>
                                    <option value="<?= $val ?>" <?= $selected ?>><?= $val ?></option>
                                <?php endforeach; ?>
                            </select>
                            <select class="cngOffice validate[required]" name="<?= $svcPrefix ?>[<?= $i ?>][service_detail_id]" style="width:200px;">
                                <option value="">選択してください</option>
                                <?php foreach ($svcDtlMst as $type => $svcDtlMst2) : ?>
                                    <?php foreach ($svcDtlMst2 as $tgtId => $val) : ?>
                            ...
                            <input type="text" name="<?= $svcPrefix ?>[<?= $i ?>][name]" style="width:90px;" value="<?= isset($svcData['name']) ? $svcData['name'] : '' ?>">
                            <p class="list_delete row_delete">Delete</p>
                        </li>
                        <?php $detail_i = $detail_i + 1; ?>
                    <?php endforeach; ?>
    ...
    <div class="s_constrols">
        ...
        <p>
            <button type="submit" name="btnChgUserRcd" value="<?= $planId ?>" class="btn save" style="width:60px;height:37px;font-size: 15px;">保存</button>
        </p>
    </div>
    <!-- 下部ボタンエリア:end-->
    <div class="update">
    ...
    <script>
        ...
        var dtlOptions = "";
        // optionをコピーする
        var sv_base_service_code = document.getElementById("serviceList");
        var baseData = sv_base_service_code.querySelectorAll("option");
        var sv_addList = document.getElementById("add_root");
        var sv_dtlList = document.getElementById("dtl_root");
        var addSelect = sv_addList.querySelectorAll("select");
        var addData = addSelect[0].querySelectorAll("option");
        var dtlSelect = sv_dtlList.querySelectorAll("select");
        var dtlData = dtlSelect[0].querySelectorAll("option");
