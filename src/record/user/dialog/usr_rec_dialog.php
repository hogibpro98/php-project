...
// 実績ID
$recordId = filter_input(INPUT_GET, 'id');
...
<div class="dynamic_modal new_default sched_default displayed_part cancel_act" style="height:600px;width:950px!important;overflow: scroll!important;overflow-y: auto;overscroll-behavior-y: contain;top:60%;">
    ...
    <div class="sched_tit">利用者スケジュール実績編集</div>
    <!-- 基本情報表示エリア:start -->
    <div class="s_detail">
        ...
        <?php $mainPrefix = "upUserRcd[" . $recordId . "]"; ?>
        <?php $addPrefix = "upUserRcdAdd[" . $recordId . "]"; ?>
        <?php $jpiPrefix = "upUserRcdJpi[" . $recordId . "]"; ?>
        <?php $svcPrefix = "upUserRcdSvc[" . $recordId . "]"; ?>
        <?php $detail_i = 1; ?>
        <?php $jippi_i = 1; ?>

        <input type="hidden" name="<?=$mainPrefix ?>[unique_id]" value="<?= $recordId ?>">
        <div class="box1">
        ...
        <ol id="add_root">
            ...
            <?php else : ?>
                <?php foreach ($dispData['add'] as $addData) : ?>
                    ...
                    <input type="hidden" name="<?=$addPrefix ?>[<?= $addId ?>][user_record_id]" value="<?= $recordId ?>">
    ...
    <div class="s_constrols">
        ...
        <p>
            ...
            <button type="submit" name="btnDelUserRcd" value="<?= $recordId ?>" class="btn delete" style="width:60px;height:37px;font-size: 15px;">削除</button>
            <button type="submit" name="btnEditUserRcd" value="<?= $recordId ?>" class="btn save" style="width:60px;height:37px;font-size: 15px;">保存</button>
        </p>
    </div>
    <!-- 下部ボタンエリア:end-->
    <div class="update">
