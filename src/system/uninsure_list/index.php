...
<?php endforeach; ?>
    ...
    <div class="office_input">
        <span class="label_t text_blue">使用事業所</span>
        <select name="sAry[office]" style="width: auto;">
            <option <?= !empty($dispSearch['office']) && $dispSearch['office'] == '' ? 'selected' : '' ?>></option>
            <?php foreach ($dispOfficeList as $val): ?>
                <?php $select = (!empty($dispSearch['office']) && $dispSearch['office'] == $val['name']) ? 'selected' : ''; ?>
	...
	<button type="submit" name="btnClear" value="true" class="btn clear" style="padding:11px;">クリア</button>
	<div class="btn add" style="padding:4px; padding-left:11px;" ><a href="/system/uninsure_edit">新規登録</a></div>
</div>
</form>
...
<div class="wrap">
	...
	<table class="main_list">
		<tr>
			...
			<th class="tax_rate" style="min-width:30px !important;">税率</th>
			<th class="deductible">控除対象</th>
			<th class="office">使用事業所</th>
			<th class="modified"  style="min-width:130px !important;">更新日時/ユーザ</th>
                        <th></th>
                        <th></th>
		</tr>
<?php foreach ($dispData as $tgtId => $val): ?>
		<tr>
			...
			<td><?= $val['price'] . '円' ?></td>
			<td><?= $val['zei_type'] ?></td>
			<td style="min-width:50px !important;"><?= $val['rate'] . '％' ?></td>
			<td><?= $val['subsidy'] ?></td>
			<td><?= $val['office'] ?></td>
			<td style="width:120px"><?= $val['update_day'] . ' ' . $val['update_time'] ?><br/><?= $val['update_name'] ?></td>
			<td><div class="btn edit" style="min-width:50px"><a href="/system/uninsure_edit?id=<?= $tgtId ?>">編集</a></div></td>
			<td><button type="submit" name="btnDelete" value="true" class="btn delete"  style="margin-left:5px;height:40px;width:58px;">削除</button></div></td>
			</form>
		</tr>
<?php endforeach; ?>
	</table>
