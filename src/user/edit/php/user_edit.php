...
if ($btnEntry){
    ...
    if ($tab == 1){
        ...
        //    }
        // 契約事業所
        if (empty($userId) && empty($upOfc1)){
                $_SESSION['notice']['error'][] = '事業所IDの指定がありません';
                $_SESSION['notice']['error'][] = '事業所名称の指定がありません';
        }else if(empty($userId) && !empty($upOfc1)){
            foreach($upOfc1 as $idx => $ofc){
            // 新規登録時
                if (empty($ofc['office_id'])){
                $_SESSION['notice']['error'][] = '事業所IDの指定がありません';
            }
                if (empty($ofc['office_name'])){
                $_SESSION['notice']['error'][] = '事業所名称の指定がありません';
            }
            }
        } else {
    ...
    if ($tab == 2) {
        ...
        // 支払方法
        if (empty($upPay['method'] ?? '')) {
            $_SESSION['notice']['error'][] = '支払方法の指定がありません';
        } else {

            // 支払方法が引き落としの場合のみ入力チェック
            if ($upPay['method'] == '引き落とし') {

                // 金融期間区分
                if (empty($upPay['bank_type'] ?? '')) {
                    $_SESSION['notice']['error'][] = '金融機関区分の指定がありません';
                }
                // 金融機関コード
                if (empty($upPay['bank_code'] ?? '')) {
                    $_SESSION['notice']['error'][] = '金融機関コードの指定がありません';
                }
                // 金融機関名
                if (empty($upPay['bank_name'] ?? '')) {
                    $_SESSION['notice']['error'][] = '金融機関名の指定がありません';
                }
                // 支店コード
                if (empty($upPay['branch_code'] ?? '')) {
                    $_SESSION['notice']['error'][] = '支店コードの指定がありません';
                }
                // 支店名
                if (empty($upPay['branch_name'] ?? '')) {
                    $_SESSION['notice']['error'][] = '支店名の指定がありません';
                }
                // 口座番号
                if (empty($upPay['deposit_code'] ?? '')) {
                    $_SESSION['notice']['error'][] = '口座番号の指定がありません';
                }
                // 預金者名
                if (empty($upPay['deposit_name'] ?? '')) {
                    $_SESSION['notice']['error'][] = '預金者名の指定がありません';
                }
                // 預金種別
                if (empty($upPay['deposit_type'] ?? '')) {
                    $_SESSION['notice']['error'][] = '預金種別の指定がありません';
                }
            }
        }
    }
...
//     $upPay  = formatDelAry('mst_user_pay',$upPay);
    $upOfc1 = formatDelAry('mst_user_office1',$upOfc1);
...
if ($btnEntry && $upPay && $tab == 2){
    ...
    $upPay['bank_type'] = $upPay['bank_type'] ?? '';
    $upPay['bank_code'] = $upPay['bank_code'] ?? '';
    $upPay['bank_name'] = $upPay['bank_name'] ?? '';
    $upPay['branch_code'] = $upPay['branch_code'] ?? '';
    $upPay['branch_name'] = $upPay['branch_name'] ?? '';
    $upPay['deposit_type'] = $upPay['deposit_type'] ?? '';
    $upPay['deposit_code'] = $upPay['deposit_code'] ?? '';
    $upPay['deposit_name'] = $upPay['deposit_name'] ?? '';
