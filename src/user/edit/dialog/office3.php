...
$tgtId = filter_input(INPUT_GET, 'tgt_id');
...
/* -- 画面表示データ格納 ---------------------------- */
?>
<div class="dynamic_modal modal_office3 cancel_act cont_user-dup" style="height:600px;z-index: 3; display:block;">
    ...
    <div class="tab-main-box">
        <table>
            <tr class="tr2" style="border:0;">
                <td>
                    <div>
                        <span class="n_search office_search3" data-tgt_id="<?= $idx ?>">Search</span>
                        <input type="hidden" name="upOfc3[<?= $idx ?>][unique_id]" id="user_office_id3" value="<?= $userOfcId ?>" class="user_office_id3" maxlength="8" pattern="^[a-zA-Z0-9]+$">
                        <input type="hidden" name="upOfc3[<?= $idx ?>][office_id]" id="office_id3" value="<?= $officeId ?>" class="office_id3" maxlength="8" pattern="^[a-zA-Z0-9]+$">
                        <input type="text" name="upOfc3[<?= $idx ?>][office_no]" id="office_no3" value="<?= $officeNo ?>" class="office_no3" maxlength="8" pattern="^[a-zA-Z0-9]+$">
                        <input type="text" name="upOfc3[<?= $idx ?>][office_name]" id="office_name3" value="<?= $officeName ?>" class="tgt-name2_<?= $idx ?> bg-gray3 office_name3" readonly>
                    </div>
                </td>
            </tr>
            <tr style="border:0;">
                <td>
                    <div>
                        ...
                        <input type="date" name="upOfc3[<?= $idx ?>][start_day]" class="date_start3" value="<?= $startDay ?>" >
                        <small>～</small>
                        <input type="date" name="upOfc3[<?= $idx ?>][end_day]" class="date_end3 " value="<?= $endDay ?>">
                    </div>
                </td>
            </tr>
        </table>
        ...
        <table>
            ...
            <tbody>
                <?php foreach ($ofcList as $ofcId => $val): ?>
                    <tr>
                        <td><button type="button" class="select_office3" data-id="<?= $val['unique_id'] ?>" data-office_id="<?= $val['unique_id'] ?>"  data-number="<?= $val['office_no'] ?>" data-name1="<?= $val['place_name'] ?>" data-name2="<?= $val['name'] ?>">選択</button></td>
...
//            $('.btn_add_office_his').off('click');
            $('.btn_add_office_his').on('click', function () {
                ...
                if(!dateStart3){
                    dateStart3 = $(document).find('.date_start3').val();
                }
                if(!dateEnd3){
                    dateEnd3 = $(document).find('.date_end3').val();
                }
        ...
        var keyId = 0;
        keyId = $(".office_history").find("li").length + 1;
        ...
        console.log("officeId:"+officeId);
            ...
            var addCont = '';
            addCont += '<li style="display:flex;" class="history_' + keyId + '">';
            ...
            addCont += '            name="" value=""';
            addCont += '            data-url="/user/edit/dialog/office3.php?user_office_id=' + uniqueId + '&office_name=' + officeName + '&office_id=' + officeId + '&start_day=' + startDay + '&end_day=' + endDay + '&tgt_id=history_' + keyId + '&mode=edit"';
